...
describe Sisimai::Rhost::NTTDOCOMO do
  ...
  describe 'bounce mail from NTT DOCOMO' do
    rs.each_key.each do |n|
      emailfn = sprintf('./set-of-emails/maildir/bsd/rhost-nttdocomo-%02d.eml', n)
      next unless File.exist?(emailfn)

      mailbox = Sisimai::Mail.new(emailfn)
      mtahost = %r/mfsmax[.]docomo[.]ne[.]jp/
      next unless mailbox
      ...
      while r = mailbox.data.read do
        ...
        mesg.ds.each do |e|
          ...
          example('agent is ' + e['agent']) { expect(e['agent']).to match(/(?:Postfix|Sendmail)/) }
        end
