...
describe 'Sisimai::CED::*' do
  ...
  CEDChildren.each_key do |x|
    ...
    describe cn do
      ...
      (1 .. CEDChildren[x].keys.size).each do |i|
        if debugOnlyTo.size > 0
          next unless debugOnlyTo == sprintf( "ced-%s-%02d", x.downcase, i)
        end

        emailfn = sprintf('./set-of-emails/maildir/bsd/ced-%s-%02d.eml', x.downcase.gsub('::', '-'), i)
        mailbox = Sisimai::Mail.new(emailfn)
        mailtxt = nil

        n = sprintf('%02d', i)
        next unless mailbox.path
        next unless CEDChildren[x][n]
        ...
        while r = mailbox.read do
          ...
          o.each do |e|
            ...
            example sprintf('[%s] %s#softbounce = %s', n, x, e.softbounce) do
              expect(e.softbounce.to_s).to match(CEDChildren[x][n]['b'])
            end
            ...
            example sprintf('[%s] %s#deliverystatus = %s', n, x, e.deliverystatus) do
              expect(e.deliverystatus).to match(CEDChildren[x][n]['s'])
            end

            example sprintf('[%s] %s#reason = %s', n, x, e.reason) do
              expect(e.reason).to match(CEDChildren[x][n]['r'])
            end
