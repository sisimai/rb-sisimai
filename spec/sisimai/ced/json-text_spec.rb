...
#     '02' => { 's' => %r/\A5[.]1[.]1\z/, 'r' => %r/userunknown/, 'b' => %r/\A0\z/ },
      '03' => { 's' => %r/\A\z/,          'r' => %r/feedback/,    'b' => %r/\A-1\z/ },
      '04' => { 's' => %r/\A2[.]6[.]0\z/, 'r' => %r/delivered/,   'b' => %r/\A-1\z/ },
      '05' => { 's' => %r/\A2[.]6[.]0\z/, 'r' => %r/delivered/,   'b' => %r/\A-1\z/ },
    },
    'US::SendGrid' => {
      '01' => { 's' => %r/\A5[.]0[.]\d+\z/, 'r' => %r/filtered/,    'b' => %r/\A1\z/ },
      '02' => { 's' => %r/\A5[.].[.]\d+\z/, 'r' => %r/(?:mailboxfull|filtered)/, 'b' => %r/\A1\z/ },
      '03' => { 's' => %r/\A5[.]1[.]1\z/,   'r' => %r/userunknown/, 'b' => %r/\A0\z/ },
      '04' => { 's' => %r/\A5[.]0[.]\d+\z/, 'r' => %r/filtered/,    'b' => %r/\A1\z/ },
      '05' => { 's' => %r/\A5[.]0[.]\d+\z/, 'r' => %r/filtered/,    'b' => %r/\A1\z/ },
      '06' => { 's' => %r/\A5[.]1[.]1\z/,   'r' => %r/userunknown/, 'b' => %r/\A0\z/ },
      '07' => { 's' => %r/\A5[.]2[.]1\z/,   'r' => %r/filtered/,    'b' => %r/\A1\z/ },
      '08' => { 's' => %r/\A5[.]1[.]1\z/,   'r' => %r/userunknown/, 'b' => %r/\A0\z/ },
      '09' => { 's' => %r/\A5[.]1[.]1\z/,   'r' => %r/userunknown/, 'b' => %r/\A0\z/ },
      '10' => { 's' => %r/\A5[.]1[.]1\z/,   'r' => %r/userunknown/, 'b' => %r/\A0\z/ },
      '11' => { 's' => %r/\A5[.]0[.]\d+\z/, 'r' => %r/hostunknown/, 'b' => %r/\A0\z/ },
    },
  }

  JSONObjects.each_key do |x|
    cn = Module.const_get('Sisimai::CED::' + x)
    ...
    describe cn do
      ...
      describe '.adapt' do
        it('returns nil') { expect(cn.adapt(nil)).to be nil }
      end

      (1 .. JSONObjects[x].keys.size).each do |i|
        if debugOnlyTo.size > 0
          next unless debugOnlyTo == sprintf( "ced-%s-%02d", x.downcase, i)
        end

        cedjson = sprintf('./set-of-emails/jsonapi/ced-%s-%02d.json', x.downcase.gsub('::', '-'), i)
        jsontxt = File.open(cedjson).read
        jsonobj = nil
        bounces = []

        n = sprintf('%02d', i)
        next unless JSONObjects[x][n]

        example sprintf('[%s] %s/mail = %s', n, cn, cedjson) do
          expect(File.exist?(cedjson)).to be true
        end
        ...
        bounces.each do |e|
          p = Sisimai::Message.new( data: e, delivered: true, input: 'json' )
          ...
          p.ds.each do |e|
            ...
            if e['action']
              example sprintf('[%s] %s[action] = %s', n, x, e['action']) do
                expect(e['action']).to match(/\A(?:fail.+|delayed|expired|deliverable)\z/)
              end
            end
          end

          o = Sisimai::Data.make( data: p, delivered: true, input: 'json' )
          it 'returns Array' do
          ...
          o.each do |e|
            ...
            example sprintf('[%s] %s#softbounce = %s', n, x, e.softbounce) do
              expect(e.softbounce.to_s).to match(JSONObjects[x][n]['b'])
            end
            ...
            example sprintf('[%s] %s#deliverystatus = %s', n, x, e.deliverystatus) do
              expect(e.deliverystatus).to match(JSONObjects[x][n]['s'])
            end

            example sprintf('[%s] %s#reason = %s', n, x, e.reason) do
              expect(e.reason).to match(JSONObjects[x][n]['r'])
            end
            ...
            example sprintf('[%s] %s#feedbacktype = %s', n, x, e.feedbacktype) do
              expect(e.feedbacktype).to be_a String
            end
