...
Callback feature
---------------------------------------------------------------------------------------------------
`:c___` argument of `Sisimai.make` and `Sisimai.dump` is an Array and is a parameter to receive Proc
...
### For each email file
Callback method set in the second element of `:c___` is called at `Sisimai.make()` method for dealing
each email file.
