...
#           It may include address syntax or validity. These errors can generally
...
#           Mail system status indicates that something having to do with the destination
#           system has caused this DSN. System issues are assumed to be under the
#           general control of the destination system administrator.
#
...
#           such as directory and routing services. Network issues are assumed to
#           be under the control of the destination or intermediate system administrator.
#
...
#           problems resulting from implementation errors or an unreliable connection.
#
# X.6.X --- Message Content or Media Status
#           The message content or media status codes report failures involving
#           the content of the message. These codes report failures due to translation,
#           transcoding, or otherwise unsupported message media. Message content
#           or media issues are under the control of both the sender and the receiver,
...
#           of either or both the sender and recipient. Both the sender and recipient
#           must permit the exchange of messages and arrange the exchange of necessary
#           keys and certificates for cryptographic operations.
#
# ------------------------------------------------------------------------------
...
#                   the left of the "@" sign is invalid. This code is only useful
...
#                   for mail. This code is only useful for permanent failures.
#
...
#                   on the destination system. This may result if a heuristic address
...
#                   is no longer being accepted for that address. This code is only
...
#        501        The sender's system specified in the address does not exist
...
#        452        The mailbox is a mailing list address and the mailing list
#                   was unable to be expanded. This code may represent a permanent
#                   failure or a persistent transient failure.
# ------------------------------------------------------------------------------
# X.3.0  221    Other or undefined mail system status:(RFC 3463)
#        250        The destination system exists and normally accepts mail, 
#        421,451    but something about the system has caused the generation of
#        550,554    this DSN.
#
...
#                   the remote system was busy, or was unable to take a call. This
...
#        503,530    out of sequence or unsupported. This is useful only as a permanent
#        550,554    error.
#        555
#
# X.5.2  500    Syntax error:(RFC 3463)
#        500,501    A mail transaction protocol command was issued which could not
#        502,550    be interpreted, either because the syntax was wrong or the command
#        555        is unrecognized. This is useful only as a permanent error.
#
...
#        501-504    A valid mail transaction protocol command was issued with invalid
#        550        arguments, either because the arguments were out of range or
#        555        represented unrecognized features. This is useful only as a
#                   permanent error.
#
...
#                   if the sender has indicated that conversion with loss is prohibited
#                   for the message.
#
...
#                   This indicates that external strong privacy layer is needed
#                   in order to use the requested authentication mechanism. This
#                   is primarily intended for use with clear text authentication
#                   mechanisms. A client which receives this may activate a security
#                   layer such as TLS prior to authenticating, or attempt to use
...
#                   account (e.g., due to lack of payment, abuse, evidence of a break-in
#                   attempt, etc). This error code occurs after a successful authentication
#                   to a disabled account. This informs the client that the failure
#                   is permanent until the user contacts their system administrator
#                   to get the account re-enabled. It differs from a generic authentication
#                   failure where the client's best option is to present the passphrase
...
#        4xx        The message is too big for the specified priority. This condition
#        5xx        might be temporary, for example the server is operating in a mode
...
#                   because the required timestamp was not recorded. The message
#                   originator needs to decide whether to reissue the message without
#                   RRVS protection.
#
...
#                   found in the From header field. This is a special case of X.7.21.
#                   (This violates the advice of Section 6.1 of [RFC6376].)
#
